#!/usr/bin/env python
"""Compute DRAM-v amg_flags and is_transposon columns.

No VirSorter input required. Adds:
  ...
  - amg_flags (str): concatenated single-letter flags in v1 order
    ...
    NOT used by any downstream filter.

Flag semantics ported from DRAM v1 mag_annotator/annotate_vgfs.py
(commit 6cd68f9), except N, which has no v1 equivalent.

The v1 auxiliary_score is not computed here. It is a flanking-context
score that needs per-gene viral categories from a virus identifier, and
is added separately.
"""
...
_ID_EXPR_DICT["vogdb_ids"] = (
    pl.col("vogdb_ids").cast(pl.Utf8).str.extract_all(_VOG_ID_RE)
)
...
def build_metabolic_genes(distill_sheets_dir: Path) -> set[str]:
    ...
    for f in sorted(distill_sheets_dir.glob("distill_*.tsv")):
        ...
        if "potential_amg" not in df.columns or "gene_id" not in df.columns:
            continue
        sub = df.filter(
            pl.col("potential_amg").cast(pl.Utf8).str.strip_chars().str.to_uppercase()
            == "TRUE"
        )
...
def compute_flags(
    ...
    df = df.with_columns(pl.col("is_transposon").any().over("scaffold").alias("_T"))
...
@click.option(
    "--length_from_end",
    default=DEFAULT_LENGTH_FROM_END,
    type=int,
    show_default=True,
...
def main(
    ...
    distill_sheets_dir = (
        Path(distill_sheets_dir)
        if distill_sheets_dir
