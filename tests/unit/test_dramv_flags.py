...
def test_individual_flags_fire():
    ...
    assert (
        flags["g5"] == "MKTF"
    )  # K forces M; T from own pfam; F near end; no B (g4 broke triple)

    # is_transposon must only be True for the row carrying the trigger pfam.
    transposons = {
        row["query_id"]: row["is_transposon"] for row in out.iter_rows(named=True)
    }
    assert transposons == {
        "g1": False,
        "g2": False,
...
def test_k_flag_forces_m():
    ...
    flags = _flags_by_query(out)
    assert flags["g1"].startswith("MK"), (
        f"expected M before K (v1 order), got {flags['g1']!r}"
    )
